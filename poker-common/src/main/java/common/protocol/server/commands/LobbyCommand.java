...
@AllArgsConstructor
@NoArgsConstructor
public class LobbyCommand implements ServerCommand {
    ...
    public void parseFromString(String input) {
        ...
        if (pairs.length == 2 && pairs[0].equals("PLAYERS")) {
            List<PlayerId> playerIds = Arrays.stream(pairs[1].split(","))
                    .map(PlayerId::fromString)
    ...
    public String toReadableString(){
        ...
        stringBuilder.append(players.stream().map(PlayerId::toString).collect(Collectors.joining(",")));
        stringBuilder.append("]");
        stringBuilder.append("\n");
