...
@AllArgsConstructor
@NoArgsConstructor
public class TurnPlayerCommand implements ServerCommand {
    private GameId gameId;
    private PlayerId playerId;
    private ServerActions action;
    private PlayerId turnPlayerId;
    ...
    public void parseFromString(String input) {
        ...
        if (pairs.length == 2 && pairs[0].equals("PLAYER")) {
            setTurnPlayerId(PlayerId.fromString(pairs[1]));
        }
    }
    ...
    public String toReadableString(){
        ...
        stringBuilder.append(turnPlayerId);
        stringBuilder.append("]");
        stringBuilder.append("\n");
