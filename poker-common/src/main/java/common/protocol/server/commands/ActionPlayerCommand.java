...
@AllArgsConstructor
@NoArgsConstructor
public class ActionPlayerCommand implements ServerCommand {
    private GameId gameId;
    private PlayerId playerId;
    private ServerActions action;
    private PlayerId actingPlayerId;
    ...
    public void parseFromString(String input) {
        ...
        if (pairs.length == 2 && pairs[0].equals("PLAYER")) {
            setActingPlayerId(PlayerId.fromString(pairs[1]));
        }
    }
    ...
    public String toReadableString(){
        ...
        stringBuilder.append(actingPlayerId);
        stringBuilder.append("]");
        stringBuilder.append("\n");
