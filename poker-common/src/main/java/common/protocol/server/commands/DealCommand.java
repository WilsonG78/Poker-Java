...
@AllArgsConstructor
@NoArgsConstructor
public class DealCommand implements ServerCommand {
    ...
    public void parseFromString(String input) {
        ...
        if (pairs.length == 2 && pairs[0].equals("CARDS")) {
            List<Card> cardList = Arrays.stream(pairs[1].split(","))
                    .map(cardString -> {
    ...
    public String toReadableString(){
        ...
        stringBuilder.append(cards.stream().map(card -> "" + card.getRank().toString() + card.getSuit().getSymbol()).collect(Collectors.joining(",")));
        stringBuilder.append("]");
        stringBuilder.append("\n");
